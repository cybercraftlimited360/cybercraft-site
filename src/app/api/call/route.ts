...
- Never reveal you use Vapi, Groq, or any underlying technology.`;
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!apiKey) return NextResponse.json({ error: "Vapi API key not configured" }, { status: 500 });
    if (!phoneNumberId) return NextResponse.json({ error: "Vapi phone number ID not configured" }, { status: 500 });
    ...
    const res = await fetch(VAPI_API, {
      ...
      body: JSON.stringify({
        phoneNumberId,
        customer: { number: e164, name: leadName },
        assistant: {
          name: "Aria",
          firstMessage: `Hi, is this ${firstName}? Great — this is Aria calling from CyberCraft360. You were just on our website looking into AI solutions for ${leadCompany}, so I wanted to reach out personally. Do you have two minutes?`,
          model: {
            provider: "groq",
            model: "llama-3.3-70b-versatile",
            messages: [{ role: "system", content: buildSystemPrompt(leadName, leadCompany, leadChallenge) }],
            maxTokens: 150,
            temperature: 0.7,
          },
          voice: {
          ...
          endCallPhrases: ["goodbye", "bye", "talk later", "thanks bye", "take care"],
    ...
    return NextResponse.json({ ok: true, callId: data.id });
  } catch (err) {
