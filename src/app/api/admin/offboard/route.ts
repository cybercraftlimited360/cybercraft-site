...
export async function POST(req: NextRequest) {
  ...
  logActivity({
    type: "cancellation",
    title: `Client offboarded — ${clientName}`,
  ...
  sendEmail({
    ...
    subject: `📤 Client Offboarded — ${clientName}`,
    html: `<div style="font-family:system-ui;padding:24px;background:#0a0c12;border-radius:12px;max-width:480px;">
      ...
      <h2 style="color:#fff;margin:0 0 20px;font-size:18px;">📤 Client Offboarded</h2>
      <p style="color:rgba(255,255,255,0.6);font-size:14px;margin:0 0 8px;"><strong style="color:#fff;">${clientName}</strong>${clientEmail ? ` · ${clientEmail}` : ""}</p>
