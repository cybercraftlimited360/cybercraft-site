...
async function sendConfirmationEmails(booking: {
  ...
  await sendEmail({
    to: booking.email,
    subject: `Your Strategy Session is Confirmed â€” ${dateLabel}`,
    html: `
<div style="background:#0a0c12;padding:40px 20px;font-family:'Inter',system-ui,sans-serif;">
    ...
    <div style="padding:36px;">
      ...
      <div style="background:rgba(0,212,255,0.05);border:1px solid rgba(0,212,255,0.2);border-radius:12px;padding:20px 24px;margin-bottom:28px;">
        <div style="margin-bottom:12px;">
          <div style="font-size:11px;color:rgba(255,255,255,0.35);text-transform:uppercase;letter-spacing:0.1em;margin-bottom:4px;">ðŸ“… Date</div>
          <div style="color:#fff;font-weight:600;font-size:15px;">${dateLabel}</div>
        </div>
        <div>
          <div style="font-size:11px;color:rgba(255,255,255,0.35);text-transform:uppercase;letter-spacing:0.1em;margin-bottom:4px;">â° Time</div>
      ...
      <a href="https://cybercraft360.com/reschedule/${booking.id}" style="display:inline-block;padding:10px 22px;border-radius:9px;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.12);color:rgba(255,255,255,0.6);font-size:12px;font-weight:600;text-decoration:none;margin-bottom:24px;">ðŸ”„ Need to reschedule?</a>
      <p style="font-size:11px;color:rgba(255,255,255,0.2);margin:0;">CyberCraft360 Â· Bespoke AI Agency Â· Houston, TX</p>
    </div>
  </div>
</div>`,
  });
  ...
  await sendEmail({
    ...
    subject: `ðŸ“… New Booking â€” ${booking.name} (${booking.company}) â€” ${dateLabel} at ${fmt12(booking.time)} CT`,
    html: `
<div style="font-family:system-ui,sans-serif;padding:32px;background:#f9fafb;max-width:560px;">
  ...
  <table style="width:100%;border-collapse:collapse;">
    ...
    <tr><td style="padding:8px 0;color:#666;">Phone</td><td style="padding:8px 0;color:#111;">${booking.phone || "â€”"}</td></tr>
...
export async function POST(req: NextRequest) {
  try {
    ...
    const gcalId  = await createCalendarEvent({
      summary: `Strategy Session â€” ${name} (${company || "CyberCraft360"})`,
    ...
    try {
      ...
      // Booking is still saved â€” email failure does not cancel the booking
    }

    import("@/lib/activity").then(({ logActivity }) =>
      logActivity({ type: "booking", title: `Booking confirmed â€” ${booking.name}`, detail: `${booking.date} at ${booking.time} Â· ${booking.company}`, clientName: booking.name })
    ).catch(() => {});
